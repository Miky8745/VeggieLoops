...
src/
  ...
  lib/
    ...
    components/
      ...
      DragTooltip.svelte     — dumb/presentational "KEY: value" box, `position: absolute; bottom: 100%` relative to its parent, solid background. Rendered by `Dial`/`ScrollField` while `dragging` is true — see below.
      Dial.svelte            — circular 0-1 knob, drag-to-set (mousedown/mousemove/mouseup via `svelte:window`). Props include `tooltipKey` (e.g. "PAN") and `format` (value → display string, defaults to percentage) which drive a `DragTooltip` shown while dragging.
